...
echo "Testing Soft Delete Functionality for Locations"
echo "=============================================="
...
# Extract location UUID from response (you'll need to adjust this based on actual response format)
...
# Test 2: Get the location
echo -e "\n2. Getting the created location..."
GET_RESPONSE=$(curl -s -X GET "$BASE_URL/locations/$LOCATION_UUID")
echo "Get Response: $GET_RESPONSE"

# Test 3: Delete the location (soft delete)
echo -e "\n3. Soft deleting the location..."
DELETE_RESPONSE=$(curl -s -X DELETE "$BASE_URL/locations/$LOCATION_UUID" \
  -H "Authorization: Bearer YOUR_TOKEN_HERE")
echo "Delete Response: $DELETE_RESPONSE"

# Test 4: Try to get the deleted location (should not be found)
echo -e "\n4. Trying to get the deleted location (should not be found)..."
GET_DELETED_RESPONSE=$(curl -s -X GET "$BASE_URL/locations/$LOCATION_UUID")
echo "Get Deleted Response: $GET_DELETED_RESPONSE"

# Test 5: Get all locations (deleted location should not appear)
echo -e "\n5. Getting all locations (deleted location should not appear)..."
